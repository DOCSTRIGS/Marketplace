...
const LocationPicker = ({ onLocationSelect, initialLocation }) => {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="h-[250px] w-full rounded-2xl overflow-hidden border-2 border-[#EEEEEE] relative">
                <Map
                ...
                <div className="absolute bottom-2 left-2 right-2 bg-white/80 backdrop-blur-sm p-2 rounded-lg pointer-events-none text-[9px] font-bold text-gray-500 text-center uppercase">
        ...
        <div className="min-h-screen bg-[#FDFDFD] font-sans text-gray-900 pb-20">
            ...
            <div className="max-w-7xl mx-auto px-6 pt-12">
                <header className="mb-12">
                    <h1 className="text-[42px] font-black tracking-tight text-gray-900 mb-2">Créer ma boutique</h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                    {/* Form Side */}
                    <div className="lg:col-span-7 bg-[#F9F9F9] rounded-[40px] p-10 border border-gray-100">
                        <form onSubmit={handleSubmit} className="space-y-10">
                            
                            {/* Logo Section */}
                            <div className="flex items-center gap-10">
                                <label className="relative group cursor-pointer">
                                    <div className="w-32 h-32 rounded-full border-2 border-dashed border-[#B03A2E] flex flex-col items-center justify-center bg-white transition-all group-hover:bg-red-50 overflow-hidden">
                                        {logoPreview ? (
                                ...
                                <div>
                                    <h4 className="font-bold text-gray-800 mb-1">Logo de la boutique</h4>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-[#EEEEEE] border-none rounded-xl py-4 px-6 text-sm focus:ring-2 focus:ring-[#8B4513] transition-all"
                                        value={data.name}
                                        onChange={e => setData('name', e.target.value)}
                                    />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Slogan</label>
                                    <input 
                                        type="text" 
                            ...
                            {/* Description */}
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Description (Biographie de l'artisan)</label>
                                <textarea 
                                    placeholder="Partagez votre histoire et votre savoir-faire..."
                                    className="w-full bg-[#EEEEEE] border-none rounded-xl py-4 px-6 text-sm focus:ring-2 focus:ring-[#8B4513] transition-all h-32 resize-none"
                            ...
                            {/* WhatsApp */}
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Téléphone WhatsApp</label>
                                <div className="flex bg-[#EEEEEE] rounded-xl overflow-hidden">
                                    <span className="py-4 px-4 text-sm text-gray-500 border-r border-gray-200">+228</span>
                                    <input 
                                        type="text" 
                                        placeholder="90 00 00 00"
                                        className="w-full bg-transparent border-none py-4 px-4 text-sm focus:ring-0 transition-all"
                                        value={data.phone}
                            ...
                            {/* Categories Dynamic */}
                            <div className="space-y-4">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Catégories d'activités</label>
                                    ...
                                    {categories && categories.length > 0 ? categories.map(cat => (
                                        <button
                                            ...
                                            className={`px-6 py-3 rounded-xl text-xs font-bold border-2 transition-all flex items-center gap-2 ${
                                                ...
                                                ? 'bg-[#E5E5E5] border-[#8B4513] text-gray-900 shadow-sm'
                                                : 'bg-[#EEEEEE] border-transparent text-gray-500 hover:bg-gray-200'
                                            }`}
                                        >
                                            <div className={`w-3.5 h-3.5 rounded border ${data.categories.includes(cat.name) ? 'bg-[#8B4513] border-[#8B4513]' : 'bg-white border-gray-300'} flex items-center justify-center`}>
                                        ...
                                        ['Artisanat', 'Électronique', 'Mode', 'Gastronomie'].map(cat => (
                                            ...
                                            >
                                                <div className={`w-3.5 h-3.5 rounded border ${data.categories.includes(cat) ? 'bg-[#8B4513] border-[#8B4513]' : 'bg-white border-gray-300'} flex items-center justify-center`}>
                            ...
                            <div className="pt-6 border-t border-gray-200">
                                <div className="flex items-center justify-between mb-8">
                                    <div>
                                        <h4 className="font-bold text-gray-900">Livraison disponible</h4>
                                        <p className="text-xs text-gray-400">Activez cette option si vous proposez la livraison à domicile.</p>
                                    </div>
                                    <button 
                                        type="button"
                                        onClick={() => setData('delivery_available', !data.delivery_available)}
                                        className={`w-14 h-8 rounded-full transition-all relative ${data.delivery_available ? 'bg-[#8B4513]' : 'bg-gray-300'}`}
                                    >
                                ...
                                <div className={`grid grid-cols-2 gap-6 transition-all duration-500 ${data.delivery_available ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Frais de livraison standard (CFA)</label>
                                        <input 
                                            type="text" 
                                            className="w-full bg-[#EEEEEE] border-none rounded-xl py-4 px-6 text-sm"
                                            value={data.delivery_fee}
                                            onChange={e => setData('delivery_fee', e.target.value)}
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Zone de couverture</label>
                                        <input 
                                            type="text" 
                                            className="w-full bg-[#EEEEEE] border-none rounded-xl py-4 px-6 text-sm"
                            ...
                            <button 
                                type="submit"
                                disabled={processing}
                                className="w-[300px] py-5 bg-[#8B4513] text-white font-black rounded-2xl shadow-2xl hover:bg-[#7a2d09] transition-all transform hover:-translate-y-1 active:scale-95"
                            >
                    ...
                    <div className="lg:col-span-5 relative">
                        <div className="sticky top-32">
                            ...
                            <div className="bg-white rounded-[32px] overflow-hidden shadow-[0_30px_80px_rgba(0,0,0,0.1)] border border-gray-100">
                                ...
                                <div className="px-8 pb-8 -mt-16 relative">
                                    <div className="w-32 h-32 rounded-full border-8 border-white bg-gray-100 shadow-lg mb-4 overflow-hidden">
                                        {logoPreview ? (
                                    ...
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <div className="bg-orange-50 text-orange-600 text-[9px] font-black px-2 py-1 rounded flex items-center gap-1 border border-orange-100 whitespace-nowrap">
                                            VENDEUR VÉRIFIÉ
                                        </div>
                                    </div>

                                    <div className="flex flex-wrap gap-4 text-[10px] font-bold text-gray-500 mb-4">
                                    ...
                                    {data.categories.length > 0 && (
                                        <div className="flex flex-wrap gap-2 mb-6">
                                            {data.categories.map(cat => (
                                                <span key={cat} className="bg-gray-100 text-gray-600 text-[9px] font-black px-2 py-0.5 rounded-full uppercase">{cat}</span>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    <div className="mb-8">
                                        <div className="flex justify-between items-center mb-4">
                                            <h4 className="font-bold text-sm text-gray-900">Produits populaires</h4>
                                    ...
                                    <button disabled className="w-full py-4 bg-gray-50 text-gray-400 font-black rounded-2xl text-xs uppercase tracking-widest border border-gray-100">
            ...
            <footer className="mt-20 pt-12 border-t border-gray-100 px-6 max-w-7xl mx-auto flex justify-between items-center text-[11px] text-gray-400 font-medium">
                <div>
                    <h3 className="text-gray-900 font-bold mb-1">LoméShop</h3>
