        ...
        <SellerLayout>
            ...
            <div className="flex flex-col h-[calc(100vh-140px)]">
                <div className="mb-6 flex justify-between items-center">
                    <div>
                        <h2 className="text-2xl font-black text-gray-900 uppercase tracking-tight">Centre de Commandement Live</h2>
                        <p className="text-gray-500 font-medium">Surveillez vos livreurs en temps réel sur Lomé.</p>
                    </div>
                    <div className="flex gap-4">
                        <div className="bg-white px-4 py-2 rounded-xl border border-gray-100 shadow-sm flex items-center gap-2">
                            <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
                            <span className="text-sm font-bold text-gray-700">{couriers.length} Livreurs en ligne</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-grow grid grid-cols-1 lg:grid-cols-4 gap-6 min-h-0">
                            ...
                            {couriers.map((courier) => (
                                ...
                                >
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="font-black text-gray-900">{courier.name}</span>
                                        <span className={`text-[10px] font-black px-2 py-1 rounded-full ${
                                            courier.status === 'En route' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'
                                        }`}>
                                            {courier.status}
                                        </span>
                                    </div>
                    ...
                    <div className="lg:col-span-3 bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden relative">
                        ...
                        {/* Floating Info Card */}
                        {activeCourier && (
                            <div className="absolute bottom-6 left-6 right-6 lg:left-auto lg:right-6 lg:w-80 bg-white/95 backdrop-blur-md p-6 rounded-3xl shadow-2xl border border-white z-10 animate-in slide-in-from-bottom-4 duration-300">
                                <div className="flex items-center gap-4 mb-6">
                                    <div className="w-16 h-16 rounded-2xl bg-gray-100 overflow-hidden border-2 border-[#8B4513]/20">
                                    ...
                                    <div>
                                        <h4 className="font-black text-gray-900">{activeCourier.name}</h4>
                                        <p className="text-xs text-gray-500 uppercase font-bold tracking-widest">{activeCourier.vehicle} • ID #{activeCourier.id}28</p>
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    ...
                                    <div className="p-3 bg-gray-50 rounded-xl">
                                        <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">Charge</p>
                                        <p className="text-sm font-black text-gray-900">{activeCourier.orders} Livraisons</p>
                                    </div>
                                </div>
