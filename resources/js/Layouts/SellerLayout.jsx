...
export default function SellerLayout({ children }) {
    ...
    useEffect(() => {
        ...
        const channel = window.Echo?.private(`user.${auth.user.id}`)
            ?.listen('.MessageSent', (e) => {
                // If we're not already on the chat page, show the red dot
                if (!window.location.pathname.startsWith('/seller/chat')) {
    ...
    const navigation = [
        ...
        { name: 'Messages', href: '/seller/chat', route: 'seller.chat', icon: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z', badge: hasNewMessage },
                        ...
                        {navigation.map((item) => {
                            ...
                            return (
                                <Link
                                    ...
                                    href={item.href}
                                    onClick={() => {
                                        if (item.name === 'Messages') setHasNewMessage(false);
