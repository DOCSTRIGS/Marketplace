...
export const ProductCardSkeleton = () => (
    <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 animate-pulse">
        ...
        <div className="p-4 space-y-3">
            ...
            <div className="flex justify-between items-center pt-2">
                <div className="h-5 bg-gray-200 dark:bg-gray-800 rounded-full w-1/3" />
    ...
    <div className="flex flex-col lg:flex-row gap-12 animate-pulse">
        <div className="w-full lg:w-1/2">
            ...
            <div className="flex gap-3">
                {[1,2,3,4].map(i => (
                    <div key={i} className="w-20 h-20 bg-gray-200 dark:bg-gray-800 rounded-xl" />
                ))}
            </div>
        </div>
        <div className="flex-1 space-y-6">
            <div className="h-4 bg-gray-200 dark:bg-gray-800 rounded-full w-1/4" />
            <div className="h-8 bg-gray-200 dark:bg-gray-800 rounded-full w-3/4" />
            <div className="h-8 bg-gray-200 dark:bg-gray-800 rounded-full w-1/4" />
...
export const OrderSkeleton = () => (
    <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden animate-pulse">
        <div className="p-6 bg-gray-50/50 dark:bg-white/5 flex justify-between">
            <div className="flex gap-8">
                {[1,2,3].map(i => (
                    <div key={i} className="space-y-2">
                        <div className="h-2 bg-gray-200 dark:bg-gray-800 rounded-full w-16" />
...
export const TableSkeleton = ({ rows = 5, cols = 4 }) => (
    <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden animate-pulse">
        <div className="bg-gray-50 dark:bg-white/5 h-12 w-full" />
        <div className="p-0 divide-y divide-gray-50 dark:divide-gray-800">
            {[...Array(rows)].map((_, i) => (
                <div key={i} className="p-6 flex gap-4">
                    {[...Array(cols)].map((_, j) => (
                        <div key={j} className="h-4 bg-gray-200 dark:bg-gray-800 rounded-full flex-grow" />
                    ))}
                </div>
            ))}
        </div>
...
export const StatsSkeleton = ({ count = 3 }) => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 animate-pulse">
        {[...Array(count)].map((_, i) => (
            <div key={i} className="bg-white dark:bg-[#1e1e1e] rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-gray-800">
                <div className="h-2 bg-gray-200 dark:bg-gray-800 rounded-full w-16 mb-3" />
