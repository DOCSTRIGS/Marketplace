...
export default function CartDrawer({ isOpen, onClose }) {
    ...
    return (
        <>
            ...
            {/* Drawer */}
            <div className={`fixed inset-y-0 right-0 h-screen w-full max-w-md bg-white dark:bg-[#1a1a1a] z-[101] shadow-2xl transform transition-transform duration-500 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="flex flex-col h-full">
                    {/* Header */}
                    <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-[#FDF8F4] dark:bg-[#1e1e1e]">
                        <div>
                            <h2 className="text-2xl font-bold text-[#222222] dark:text-white">Mon Panier</h2>
                            <p className="text-sm text-gray-500 dark:text-gray-400">{cart.length} article(s) sélectionné(s)</p>
                        </div>
                        ...
                        {cart.length > 0 ? (
                            cart.map((item) => (
                                <div key={item.id} className="flex gap-4 group">
                                    <div className="w-24 h-24 rounded-xl bg-gray-100 dark:bg-[#252525] overflow-hidden flex-shrink-0">
                                        <img
                                    ...
                                    <div className="flex-grow flex flex-col">
                                        <div className="flex justify-between items-start">
                                            <h4 className="font-bold text-[#222222] dark:text-white text-sm line-clamp-1">{item.name}</h4>
                                            <button onClick={() => removeFromCart(item.id)} className="text-gray-300 dark:text-gray-600 hover:text-red-500 transition-colors">
                                        ...
                                        <div className="mt-auto flex items-center justify-between">
                                            <div className="flex items-center border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-white/5">
                                                <button
                                                    onClick={() => updateQuantity(item.id, item.quantity - 1)}
                                                    className="px-2 py-1 text-gray-900 dark:text-white hover:text-[#B03A2E] transition-colors"
                                                >
                                                    -
                                                </button>
                                            ...
                                            <span className="font-bold text-sm text-[#222222] dark:text-white">
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                                <div className="w-20 h-20 bg-[#FDF8F4] dark:bg-white/5 rounded-full flex items-center justify-center text-[#D35400]/20">
                    ...
                    {/* Footer */}
                    {cart.length > 0 && (
                        <div className="p-6 bg-white dark:bg-[#1a1a1a] border-t border-gray-100 dark:border-gray-800 space-y-4 shadow-[0_-10px_40px_rgba(0,0,0,0.04)] dark:shadow-none">
                            <div className="space-y-2">
                                ...
                                <div className="flex justify-between items-center pt-2">
                                    <span className="text-lg font-bold text-[#222222] dark:text-white">Total</span>
